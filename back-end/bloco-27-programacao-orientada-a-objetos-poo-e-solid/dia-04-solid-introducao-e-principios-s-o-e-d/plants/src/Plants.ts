...
class Plants {
  ...
  async getPlants() {
    const plantsRaw = await fs.readFile(plantsDB, { encoding: 'utf8' });
  ...
  async removePlantById(id: string) {
    ...
    await fs.writeFile(plantsDB, JSON.stringify(newPlants));
  ...
  async editPlant(plantId: string, newPlant: IPlant) {
    ...
    await fs.writeFile(plantsDB, JSON.stringify(updatedPlants));
    return newPlant;
  }
  ...
  async savePlant(plant: IPlant) {
    ...
    await fs.writeFile(plantsDB, JSON.stringify(plants));
    return newPlant;
  }
}
